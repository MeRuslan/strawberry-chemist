...
class DataLoaderContainer:
    ...
    def get_dataloader(
        ...
        if (field, options) not in self._loaders.keys():
            ...
            if (
                    isinstance(field, StrawberrySQLAlchemyRelationField)
                    and not isinstance(field, SQLAlchemyBaseConnectionField)
            ):
...
def local_key_sql_values(
        data: List[Any],
        local_cs: Set[ColumnElement]
) -> Values:
    value_expr = values(
        *[column(l_c.name, l_c.type) for l_c in local_cs],
        name="parent_values"
    ).data(data)
    return value_expr


def restrict_fields(target: DeclarativeMeta, fields: List[ColumnElement], query: Select):
    ...
    if target.__mapper__.polymorphic_identity is None:
        return query.options(
            Load(target).load_only(*[
                getattr(target, field.key) for field in fields
            ])
        )
    return query
...
def process_orm_results_for_dataload(
    ...
    res_dict = {
        key: [result[key_len] for result in group]
...
def _extract_ordered_columns(order_by_clauses: Tuple[ColumnElement, ...]) -> Set[ColumnElement]:
...
def add_primary_key_tie_breaker(query: Select, model: DeclarativeMeta) -> Select:
    ...
    missing_pk_columns = [
        ...
        if not any(proxy in ordered_columns for proxy in getattr(pk, "proxy_set", (pk,)))
    ]
...
class ChildrenLoadingStrategy:
    ...
    @staticmethod
    def apply_secondary(query: Select,
                        relationship_property: RelationshipProperty,
        ...
        assert len(relationship_property.secondary_synchronize_pairs) == 1, "raise on composites, just to check"
        remote_keys = list(filter(lambda x: x != relationship_property.secondaryjoin.right, remote_keys))
        return query.join(relationship_property.secondary, relationship_property.secondaryjoin), remote_keys
...
class ValuesLoadingStrategy(ChildrenLoadingStrategy):
    ...
    def construct_query(
        ...
        if relationship_property.secondary is not None:
            query, remote_keys = ValuesLoadingStrategy.apply_secondary(query, relationship_property, remote_keys)
        ...
        local_keys, remote_keys = zip(*sorted(
        ...
        result_q = (
            select(value_expr, related_alias)
            .select_from(
                value_expr
                .outerjoin(query, true())
            )
        )
...
class UnionLoadingStrategy(ChildrenLoadingStrategy):
    ...
    def construct_query(
        ...
        if relationship_property.secondary is not None:
            query, remote_keys = UnionLoadingStrategy.apply_secondary(query, relationship_property, remote_keys)

        assert len(remote_keys) == 1, "raise on composites, second guard"
        q_list = []
        for parent_keys in parent_data_local:
            labeled_parent_value = literal(list(parent_keys)[0]).label(UnionLoadingStrategy.literal_value_name)
...
class LoadViaParents(ABC):
    ...
    def extract_parents_keys(self, parents: List[Any]) -> Tuple[Tuple[str, ...], List[Tuple]]:
        local_columns_names: tuple[str, ...] = tuple(l_c.key for l_c in self.relationship_property.local_columns)
...
class RelationshipLoader(LoadViaParents, Callable):
    ...
    async def __call__(self, parents: List) -> List:
        ...
        res_flat = await self.load(parents=parents, fields_to_load=fields_to_load, children_query=query_base)
...
class ConnectionLoader(LoadViaParents, Callable):
    ...
    def filtered_ordered_paginated_query(self):
        ...
        if self.connection.pre_filter:
            for f in self.connection.pre_filter.eval(
                connection_loader=self
            ):
                query = query.filter(f)
    ...
    async def load_root_connection(self, fields: List[ColumnElement], query: Select):
        ...
        async with ctx.get_session() as asession:
            results = await asession.execute(result_q)
            result = self.connection.pagination.paginate_result(
                results.scalars().all()
            )
            return [result]
    ...
    async def __call__(self, parents: List[Any]):
        ...
        assert (self.relationship_property or not any(parents)), "Impossible: raise on to_load if no relationship"

        if not self.relationship_property:
            return await self.load_root_connection(fields=fields, query=query)

        res_flat = await self.load(parents=parents, fields_to_load=fields, children_query=query)

        for i in range(len(res_flat)):
            res_flat[i] = self.connection.pagination.paginate_result(
                res_flat[i]
            )
