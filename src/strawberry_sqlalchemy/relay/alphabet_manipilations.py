...
URL_CONVERSION_DATA = ConversionData(
    ...
    ALPHABET_CONVERTER={v: i for i, v in enumerate(_url_alphabet)},
)
LOWERCASE_CONVERSION_DATA = ConversionData(
    ALPHABET=string.ascii_lowercase,
    ALPHABET_SIZE=len(string.ascii_lowercase),
    ALPHABET_CONVERTER={v: i for i, v in enumerate(string.ascii_lowercase)},
)
...
def base_10_to_alphabet(number, conversion_data: ConversionData):
    ...
    return "".join(
        conversion_data.ALPHABET[part] for part in _decompose(number, conversion_data)
    )[::-1]
