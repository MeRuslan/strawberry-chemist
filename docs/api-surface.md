...
| Relay IDs and nodes | `@sc.node`, `sc.node_field`, `sc.Node`, `sc.relay.configure(...)`, `sc.relay.encode_node_id(...)`, `sc.relay.decode_node_id(...)` | Register node types and resolve them through schema-owned relay IDs | [`04_nodes_and_relay_ids`](https://github.com/MeRuslan/strawberry-chemist/tree/main/examples/04_nodes_and_relay_ids) |
