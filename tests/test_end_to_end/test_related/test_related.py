...
from tests.test_end_to_end.test_related.schema import (
    Base, Book, Person, current_year
)
...
async def relay_with_authors_books(mock_sqlite_sqla_session):
    ...
    lotr = Book(title="The Lord of the Rings", author=tolkien, year=1954, isbn="978-0261102385")
    silmarillion = Book(title="The Silmarillion", author=tolkien, year=1977, isbn="978-0007523221")
    hurin = Book(title="Children of Hurin", author=tolkien, year=2007, isbn="978-0007246229")
    ice_fire = Book(title="Song of Ice and Fire", author=grr_martin, year=1996, isbn="978-0345535528")
    harry_potter = Book(title="Harry Potter", author=None, year=1997, isbn="978-0439708180")
...
async def test_load_related(relay_with_authors_books, test_relay_client):
    ...
    assert hurin.title in [book['title'] for book in result['data']['personByName']['books']]
...
async def test_load_related_level2(relay_with_authors_books, test_relay_client):
    ...
    query = "{ personByName(name: \"%s\") { name books { title author {name} } } }" % grr_martin_name
    result = test_relay_client.post("/", json={"query": query}).json()

    assert 'errors' not in result
    # check author name
    assert grr_martin_name == result['data']['personByName']['name']
    assert grr_martin_name == result['data']['personByName']['books'][0]['author']['name']


@pytest.mark.asyncio
async def test_related_with_compound_filter(relay_with_authors_books, test_relay_client):
    author_name = tolkien.name
    query = "{ personByName(name: \"%s\") { name booksBefore1960 { title year } } }" % author_name
    result = test_relay_client.post("/", json={"query": query}).json()
    ...
    assert all(book["year"] < 1960 for book in result['data']['personByName']['booksBefore1960'])


@pytest.mark.asyncio
async def test_related_with_filter(relay_with_authors_books, test_relay_client):
    author_name = tolkien.name
    query = "{ personByName(name: \"%s\") { name booksAfter1960StartingWithThe { title year } } }" % author_name
    result = test_relay_client.post("/", json={"query": query}).json()
    ...
    assert all(book["year"] > 1960 for book in result['data']['personByName']['booksAfter1960StartingWithThe'])
    assert all(
...
@pytest.mark.asyncio
async def test_related_load_nonsql_data_dependant_field(relay_with_authors_books, test_relay_client):
    author_name = tolkien.name
    query = ("{ personByName(name: \"%s\") { name books {"
    ...
    book_the_hobbit_from_result = next(
        book for book in result['data']['personByName']['books'] if book['title'] == hobbit.title
    )
    assert book_the_hobbit_from_result['yearsSincePublished'] == current_year - hobbit.year
...
@pytest.mark.asyncio
async def test_author_with_no_books(
...
async def test_field_with_resolver_class_method(
    ...
    query = ("{ personByName(name: \"%s\") { name books {"
             " titleTwice } } }" % author_name)
    result = test_relay_client.post("/", json={"query": query}).json()
    assert 'errors' not in result
    assert book_title * 2 in [book["titleTwice"] for book in  result['data']['personByName']['books']]


@pytest.mark.asyncio
async def test_field_with_resolver_func(
        relay_with_authors_books, test_relay_client):
    author_name = grr_martin.name
    book_title = ice_fire.title

    query = ("{ personByName(name: \"%s\") { name books {"
             " titleThrice } } }" % author_name)
    result = test_relay_client.post("/", json={"query": query}).json()
    assert 'errors' not in result
    assert book_title * 3 in [book["titleThrice"] for book in  result['data']['personByName']['books']]


@pytest.mark.asyncio
async def test_relation_with_needs_fields(
        relay_with_authors_books, test_relay_client):
    author_name = tolkien.name
...
async def test_relation_with_ignore_field_selections_positive(
    ...
    query = ("{ personByName(name: \"%s\") { name bookBinaryYears { binaryYear } } }" % author_name)
    result = test_relay_client.post("/", json={"query": query}).json()
    assert 'errors' not in result
    res_binary_years = [res['binaryYear'] for res in result['data']['personByName']['bookBinaryYears']]
