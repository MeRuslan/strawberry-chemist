...
hobbit, lotr, silmarillion, hurin, ice_fire, tolkien, tolkien_jr, grr_martin, yemets, harry_potter = (
...
async def authors_books(mock_sqlite_sqla_session):
    ...
    global hobbit, lotr, silmarillion, hurin, ice_fire, tolkien, tolkien_jr, grr_martin, yemets, harry_potter
...
async def test_load_related(authors_books, test_through_client):
    ...
    # check books
    assert (set(book.title for book in tolkien.books) ==
            set(book['title'] for book in result['data']['personByName']['books']))
...
async def test_load_related_through_junior(authors_books, test_through_client):
    ...
    assert 'errors' not in result
    # check books
    assert (set(book.title for book in tolkien_jr.books) ==
            set(book['title'] for book in result['data']['personByName']['books']))


@pytest.mark.asyncio
async def test_load_related_reverse(authors_books, test_through_client):
    query = "{ bookByTitle(title: \"%s\") { title authors { name } } }" % silmarillion.title
    result = test_through_client.post("/", json={"query": query}).json()
    ...
    # check authors
    assert (set(author.name for author in [tolkien_jr, tolkien]) ==
            set(author['name'] for author in result['data']['bookByTitle']['authors']))


@pytest.mark.asyncio
async def test_load_related_through_three_levels(authors_books, test_through_client):
    query = "{ personByName(name: \"%s\") { name books { title authors { name  books { title } } } } }" % tolkien.name
    result = test_through_client.post("/", json={"query": query}).json()
    ...
    for book in result['data']['personByName']['books']:
        ...
        assert (set(author['name'] for author in book['authors']) ==
                set(a.name for a in orm_book.authors))
