...
from strawberry_sqlalchemy.loaders import UnionLoadingStrategy, LoadViaParents, process_orm_results_for_dataload, \
    RelationshipLoader
...
class Association(Base):
    ...
    left_id: Mapped[int] = mapped_column(ForeignKey("parent_through.id"), primary_key=True)
    right_id: Mapped[int] = mapped_column(ForeignKey("child_through.id"), primary_key=True)
...
def field_and_relation(mock_sqlite_sqla_session):
    ...
    rel_load = RelationshipLoader(relationship_property=OneOrManyThrough.children.prop, field=field)
    return rel_load
...
async def test_load_children_for_many_using_union_strategy_directly(data_session):
    async with data_session as session: # noqa need the same code in psql tests
        parents: List[OneOrManyThrough] = (await session.execute(select(OneOrManyThrough).where(
            or_(OneOrManyThrough.name == "Victor", OneOrManyThrough.name == "Jack", OneOrManyThrough.name == "Kamil", OneOrManyThrough.name == "Victoria")
        ).options(joinedload(OneOrManyThrough.children)))).unique().scalars().all()
...
async def test_load_children_for_many_using_loader(data_session, mock_context_var, field_and_relation):
    ...
    async with data_session as session: # noqa need the same code in psql tests
        parents_only: List[OneOrManyThrough] = (await session.execute(select(OneOrManyThrough).where(
            or_(OneOrManyThrough.name == "Jack", OneOrManyThrough.name == "Kamil")
        ).options(joinedload(OneOrManyThrough.children)))).unique().scalars().all()
        parents_with_kids: List[OneOrManyThrough] = (await session.execute(select(OneOrManyThrough).where(
            or_(OneOrManyThrough.name == "Jack", OneOrManyThrough.name == "Kamil")
        ))).unique().scalars().all()
...
async def test_load_children_for_many_using_loader_with_pre_filter(
    ...
    async with data_session as session: # noqa need the same code in psql tests
        alice: OneOrManyThrough = (await session.execute(select(OneOrManyThrough).where(OneOrManyThrough.name == "Alice"))).scalar_one()
        victor: OneOrManyThrough = (await session.execute(select(OneOrManyThrough).where(OneOrManyThrough.name == "Victor"))).scalar_one()
